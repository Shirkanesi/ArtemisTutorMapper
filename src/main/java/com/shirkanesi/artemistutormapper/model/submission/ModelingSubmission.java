...
import com.shirkanesi.artemistutormapper.model.participation.ModelingParticipation;
...
public class ModelingSubmission extends Submission {
    ...
    @JsonProperty
    private ModelingParticipation participation;

    @Override
    protected Response doLockSubmission(ArtemisClient client) throws IOException {
        String url = String.format(client.getArtemisBaseUrl() + "/api/modeling-submissions/%d?correction-round=0", this.getId());
    ...
    protected Response doCancelAssessmentOfSubmission(ArtemisClient client) throws IOException {
        String url = String.format(client.getArtemisBaseUrl() + "/api/modeling-submissions/%d/cancel-assessment", this.getId());
